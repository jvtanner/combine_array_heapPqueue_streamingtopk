...
Vector<DataPoint> merged(Vector<DataPoint> left, Vector<DataPoint> right);

/*Takes as input a list containing zero or more lists of data points,
 * then uses an algorithm to combine them into one giant sorted sequence.
 */
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    //Cut sequences in half and store each half separately
    Vector<Vector<DataPoint>> left = sequences.subList(0, half);
    Vector<Vector<DataPoint>> right = sequences.subList(half);

    //Recursively combine each Vector from each half
    Vector<DataPoint> result = combine(left);
    Vector<DataPoint> result1 = combine(right);

    //Call merged to sort the two combine vectors in an ordered way
    Vector<DataPoint> total = merged(result, result1);
    return total;
}

//Loop through each Vector simultaneously, comparing values and storing the smallest first
Vector<DataPoint> merged(Vector<DataPoint> left, Vector<DataPoint> right){
    int counterRight = 0;
    int counterLeft = 0;
    Vector<DataPoint> result;
    while (true) {
        //if loop has exhausted right, only pull from left
        if (counterRight == right.size()){
            result += left.subList(counterLeft);
            return result;
        }
        //if loop has exhausted left, only pull from right
        if (counterLeft == left.size()){
            result += right.subList(counterRight);
            return result;
        }
        //store right if left is bigger
        if (left[counterLeft].weight > right[counterRight].weight){
            result += right[counterRight];
            counterRight ++;
        //store left if right is bigger
        } else {
            result += left[counterLeft];
            counterLeft ++;
        }
    }
    return result;
...
ADD_TEST("Returns one single-item sequence.") {
    DataPoint pranav = { "Pranavasaurus Rex", 103 };

    /* Should return the input unchanged. */
    auto merged = combine({{ pranav }});
    EXPECT_EQUAL(merged.size(), 1);
    EXPECT_EQUAL(merged[0], pranav);
}
